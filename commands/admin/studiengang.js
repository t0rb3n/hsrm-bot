...
module.exports = {
    ...
    usage: "!studiengang add|remove label [emoji] [role]",
    async execute(client, message, args) {
        ...
        if (args[0] === "add") {
            if (args.length < 4) {
                await message.channel.send("Folgendes Format wird akzeptiert: !studiengang add [label] [emoji] [role]")
                return;
            }

            await addStudiengang(message, args[1], args[2], args[3].match(/([0-9]+)/)[0]);
            ...
            if (args.length < 2) {
                await message.channel.send("Folgendes Format wird akzeptiert: !studiengang remove [label]")
                return;
            }

            await studiengang.removeButtonByLabel(message, args[1]);
            ...
            await message.channel.send({content: "Usage: !studiengang add|remove label [emoji] [role]"})
        }
...
async function addStudiengang(message, label, emoji, role) {

    const foundCampus = await Button.findOne({where: {customId: `studiengang-${label}`}});
    if (foundCampus) {
